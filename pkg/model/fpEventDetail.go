...
// PMapEventDetail applies the function(1st argument) on each item in the list and returns a new list.
...
func PMapEventDetail(f func(EventDetail) EventDetail, list []EventDetail, optional ...fp.Optional) []EventDetail {
	if f == nil {
		return []EventDetail{}
	}
	...
	if optional != nil && len(optional) > 0 {
		...
		if optional[0].RandomOrder == true {
			return pMapEventDetailNoOrder(f, list, worker)
		}
	}

	return pMapEventDetailPreserveOrder(f, list, worker)
}

func pMapEventDetailPreserveOrder(f func(EventDetail) EventDetail, list []EventDetail, worker int) []EventDetail {
	chJobs := make(chan map[int]EventDetail, len(list))
	go func() {
		for i, v := range list {
			chJobs <- map[int]EventDetail{i: v}
		}
		close(chJobs)
	}()

	chResult := make(chan map[int]EventDetail, worker/3)
	...
	for i := 0; i < worker; i++ {
		...
		go func(chResult chan map[int]EventDetail, chJobs chan map[int]EventDetail) {
			defer wg.Done()

			for m := range chJobs {
				for k, v := range m {
					chResult <- map[int]EventDetail{k: f(v)}
				}
			}
		}(chResult, chJobs)
	}
	...
	newListMap := make(map[int]EventDetail, len(list))
	newList := make([]EventDetail, len(list))
...
func pMapEventDetailNoOrder(f func(EventDetail) EventDetail, list []EventDetail, worker int) []EventDetail {
	chJobs := make(chan EventDetail, len(list))
	go func() {
		for _, v := range list {
			chJobs <- v
		}
	...
	chResult := make(chan EventDetail, worker/3)
	...
	for i := 0; i < worker; i++ {
		...
		go func(chResult chan EventDetail, chJobs chan EventDetail) {
			defer wg.Done()
...
// PMapEventDetailPtr applies the function(1st argument) on each item in the list and returns a new list.
//  Order of new list is guaranteed. This feature can be disabled by passing: Optional{RandomOrder: true} to gain performance
//  Run in parallel. no_of_goroutines = no_of_items_in_list or 3rd argument can be passed to fix the number of goroutines.
//
// Takes 3 inputs. 3rd argument is optional
...
//  2. List
//  3. optional argument - fp.Optional{FixedPool: <some_number>}
func PMapEventDetailPtr(f func(*EventDetail) *EventDetail, list []*EventDetail, optional ...fp.Optional) []*EventDetail {
	if f == nil {
		return []*EventDetail{}
	}
	...
	if optional != nil && len(optional) > 0 {
		...
		if optional[0].RandomOrder == true {
			return pMapEventDetailPtrNoOrder(f, list, worker)
		}
	}

	return pMapEventDetailPtrPreserveOrder(f, list, worker)
}

func pMapEventDetailPtrPreserveOrder(f func(*EventDetail) *EventDetail, list []*EventDetail, worker int) []*EventDetail {
	chJobs := make(chan map[int]*EventDetail, len(list))
	go func() {
		for i, v := range list {
			chJobs <- map[int]*EventDetail{i: v}
...
func pMapEventDetailPtrNoOrder(f func(*EventDetail) *EventDetail, list []*EventDetail, worker int) []*EventDetail {
	chJobs := make(chan *EventDetail, len(list))
	go func() {
		for _, v := range list {
			chJobs <- v
...
// PMapEventDetailPtrErr applies the function(1st argument) on each item in the list and returns a new list and error.
//  Order of new list is guaranteed. This feature can be disabled by passing: Optional{RandomOrder: true} to gain performance
//  Run in parallel. no_of_goroutines = no_of_items_in_list or 3rd argument can be passed to fix the number of goroutines.
//
// Takes 3 inputs. 3rd argument is optional
...
//  2. List
//  3. optional argument - fp.Optional{FixedPool: <some_number>}
func PMapEventDetailPtrErr(f func(*EventDetail) (*EventDetail, error), list []*EventDetail, optional ...fp.Optional) ([]*EventDetail, error) {
	if f == nil {
		return []*EventDetail{}, nil
	}
	...
	if optional != nil && len(optional) > 0 {
		...
		if optional[0].RandomOrder == true {
			return pMapEventDetailPtrErrNoOrder(f, list, worker)
		}
	}

	return pMapEventDetailPtrErrPreserveOrder(f, list, worker)
}

func pMapEventDetailPtrErrPreserveOrder(f func(*EventDetail) (*EventDetail, error), list []*EventDetail, worker int) ([]*EventDetail, error) {
	chJobs := make(chan map[int]*EventDetail, len(list))
	go func() {
		for i, v := range list {
			chJobs <- map[int]*EventDetail{i: v}
	...
	for i := 0; i < worker; i++ {
		...
		go func(chResult chan map[int]*EventDetail, chJobs chan map[int]*EventDetail, errCh chan error) {
			defer wg.Done()
			if len(errCh) > 0 {
				return
			}
			for m := range chJobs {
				for k, v := range m {
					...
					chResult <- map[int]*EventDetail{k: r}
				}
			}
...
func pMapEventDetailPtrErrNoOrder(f func(*EventDetail) (*EventDetail, error), list []*EventDetail, worker int) ([]*EventDetail, error) {
	chJobs := make(chan *EventDetail, len(list))
	go func() {
		for _, v := range list {
			chJobs <- v
	...
	for i := 0; i < worker; i++ {
		...
		go func(chResult chan *EventDetail, chJobs chan *EventDetail, errCh chan error) {
			defer wg.Done()
			if len(errCh) > 0 {
				return
			}
...
// PMapEventDetailErr applies the function(1st argument) on each item in the list and returns a new list and error.
//  Order of new list is guaranteed. This feature can be disabled by passing: Optional{RandomOrder: true} to gain performance
//  Run in parallel. no_of_goroutines = no_of_items_in_list or 3rd argument can be passed to fix the number of goroutines.
//
// Takes 3 inputs. 3rd argument is optional
...
//  2. List
//  3. optional argument - fp.Optional{FixedPool: <some_number>}
func PMapEventDetailErr(f func(EventDetail) (EventDetail, error), list []EventDetail, optional ...fp.Optional) ([]EventDetail, error) {
	if f == nil {
		return []EventDetail{}, nil
	}
	...
	if optional != nil && len(optional) > 0 {
		...
		if optional[0].RandomOrder == true {
			return pMapEventDetailErrNoOrder(f, list, worker)
		}
	}

	return pMapEventDetailErrPreserveOrder(f, list, worker)
}

func pMapEventDetailErrPreserveOrder(f func(EventDetail) (EventDetail, error), list []EventDetail, worker int) ([]EventDetail, error) {
	chJobs := make(chan map[int]EventDetail, len(list))
	go func() {
		for i, v := range list {
			chJobs <- map[int]EventDetail{i: v}
...
func pMapEventDetailErrNoOrder(f func(EventDetail) (EventDetail, error), list []EventDetail, worker int) ([]EventDetail, error) {
	chJobs := make(chan EventDetail, len(list))
	go func() {
		for _, v := range list {
			chJobs <- v
...
// DistinctEventDetailP returns true if no two of the arguments are =
func DistinctEventDetailP(list []EventDetail) bool {
	if len(list) == 0 {
		return false
	}
...
// DistinctEventDetailPPtr returns true if no two of the arguments are =
func DistinctEventDetailPPtr(list []*EventDetail) bool {
	if len(list) == 0 {
		return false
	}
...
// DistinctEventDetail removes duplicates.
func DistinctEventDetail(list []EventDetail) []EventDetail {
	if len(list) == 0 {
		return []EventDetail{}
	}

	newList := []EventDetail{list[0]}
...
func DistinctEventDetailPtr(list []*EventDetail) []*EventDetail {
	if len(list) == 0 {
		return []*EventDetail{}
	}
...
// UnionEventDetail return a set that is the union of the input sets
// repeated value within list parameter will be ignored
func UnionEventDetail(arrList ...[]EventDetail) []EventDetail {
	var newList []EventDetail
...
// UnionEventDetailPtr return a set that is the union of the input sets
// repeated value within list parameter will be ignored
func UnionEventDetailPtr(arrList ...[]*EventDetail) []*EventDetail {
	var newList []*EventDetail
...
// IntersectionEventDetail return a set that is the intersection of the input sets
// repeated value within list parameter will be ignored
func IntersectionEventDetail(arrList ...[]EventDetail) []EventDetail {
	if arrList == nil {
		return []EventDetail{}
	}
...
// IntersectionEventDetailPtr return a set that is the intersection of the input sets
// repeated value within list parameter will be ignored
func IntersectionEventDetailPtr(arrList ...[]*EventDetail) []*EventDetail {
	if arrList == nil {
		return []*EventDetail{}
	}
...
// DifferenceEventDetail returns a set that is the first set without elements of the remaining sets
// repeated value within list parameter will be ignored
func DifferenceEventDetail(arrList ...[]EventDetail) []EventDetail {
	if arrList == nil {
		return []EventDetail{}
	}
...
// DifferenceEventDetailPtr returns a set that is the first set without elements of the remaining sets
// repeated value within list parameter will be ignored
func DifferenceEventDetailPtr(arrList ...[]*EventDetail) []*EventDetail {
	if arrList == nil {
		return []*EventDetail{}
	}
...
// SubsetEventDetail returns true or false by checking if set1 is a subset of set2
// repeated value within list parameter will be ignored, order does not matter
func SubsetEventDetail(list1, list2 []EventDetail) bool {
...
// SubsetEventDetailPtr returns true or false by checking if set1 is a subset of set2
// repeated value within list parameter will be ignored, order does not matter
func SubsetEventDetailPtr(list1, list2 []*EventDetail) bool {
	if list1 == nil || len(list1) == 0 || list2 == nil || len(list2) == 0 {
		return false
	}
...
// SupersetEventDetail returns true or false by checking if set1 is a superset of set2
// repeated value within list parameter will be ignored, order does not matter
func SupersetEventDetail(list1, list2 []EventDetail) bool {
	if list1 == nil || len(list1) == 0 || list2 == nil || len(list2) == 0 {
		return false
	}
...
// SupersetEventDetailPtr returns true or false by checking if set1 is a superset of set2
// repeated value within list parameter will be ignored, order does not matter
func SupersetEventDetailPtr(list1, list2 []*EventDetail) bool {
	if list1 == nil || len(list1) == 0 || list2 == nil || len(list2) == 0 {
		return false
	}
...
// SetEventDetail returns a set of the distinct elements of coll.
func SetEventDetail(list []EventDetail) []EventDetail {
	if len(list) == 0 {
		return []EventDetail{}
	}
...
// SetEventDetailPtr returns a set of the distinct elements of coll.
func SetEventDetailPtr(list []*EventDetail) []*EventDetail {
	if len(list) == 0 {
		return []*EventDetail{}
	}
...
type byIDEventDetail []EventDetail

func (a byIDEventDetail) Len() int           { return len(a) }
func (a byIDEventDetail) Less(i, j int) bool { return a[i].ID < a[j].ID }
func (a byIDEventDetail) Swap(i, j int)      { a[i], a[j] = a[j], a[i] }

// SortEventDetailByID sort structs
func SortEventDetailByID(list []EventDetail) []EventDetail {
	if len(list) == 0 {
		return []EventDetail{}
	}
	newList := make([]EventDetail, len(list))
	...
	sort.Sort(byIDEventDetail(newList))
	return newList
}

// SortEventDetailByIDPtr sorts structs
func SortEventDetailByIDPtr(list []*EventDetail) []*EventDetail {
	if len(list) == 0 {
		return []*EventDetail{}
	}
	newList := make([]EventDetail, len(list))
...
type byIDEventDetailDesc []EventDetail

func (a byIDEventDetailDesc) Len() int           { return len(a) }
func (a byIDEventDetailDesc) Less(i, j int) bool { return a[i].ID > a[j].ID }
func (a byIDEventDetailDesc) Swap(i, j int)      { a[i], a[j] = a[j], a[i] }

// SortEventDetailByIDDesc sorts structs
func SortEventDetailByIDDesc(list []EventDetail) []EventDetail {
	if len(list) == 0 {
		return []EventDetail{}
	}
	newList := make([]EventDetail, len(list))
	...
	sort.Sort(byIDEventDetailDesc(newList))
	return newList
}

// SortEventDetailByIDDescPtr sorts structs
func SortEventDetailByIDDescPtr(list []*EventDetail) []*EventDetail {
	if len(list) == 0 {
		return []*EventDetail{}
	}
	newList := make([]EventDetail, len(list))
...
type byUpdatedAtEventDetail []EventDetail

func (a byUpdatedAtEventDetail) Len() int           { return len(a) }
func (a byUpdatedAtEventDetail) Less(i, j int) bool { return a[i].UpdatedAt.Before(a[j].UpdatedAt) }
func (a byUpdatedAtEventDetail) Swap(i, j int)      { a[i], a[j] = a[j], a[i] }

// SortEventDetailByUpdatedAt sort structs
func SortEventDetailByUpdatedAt(list []EventDetail) []EventDetail {
	if len(list) == 0 {
		return []EventDetail{}
	}
	newList := make([]EventDetail, len(list))
	...
	sort.Sort(byUpdatedAtEventDetail(newList))
	return newList
}

// SortEventDetailByUpdatedAtPtr sorts structs
func SortEventDetailByUpdatedAtPtr(list []*EventDetail) []*EventDetail {
	if len(list) == 0 {
		return []*EventDetail{}
	}
	newList := make([]EventDetail, len(list))
...
type byUpdatedAtEventDetailDesc []EventDetail

func (a byUpdatedAtEventDetailDesc) Len() int           { return len(a) }
func (a byUpdatedAtEventDetailDesc) Less(i, j int) bool { return a[i].UpdatedAt.After(a[j].UpdatedAt) }
func (a byUpdatedAtEventDetailDesc) Swap(i, j int)      { a[i], a[j] = a[j], a[i] }

// SortEventDetailByUpdatedAtDesc sorts structs
func SortEventDetailByUpdatedAtDesc(list []EventDetail) []EventDetail {
	if len(list) == 0 {
		return []EventDetail{}
	}
	newList := make([]EventDetail, len(list))
	...
	sort.Sort(byUpdatedAtEventDetailDesc(newList))
	return newList
}

// SortEventDetailByUpdatedAtDescPtr sorts structs
func SortEventDetailByUpdatedAtDescPtr(list []*EventDetail) []*EventDetail {
	if len(list) == 0 {
		return []*EventDetail{}
	}
	newList := make([]EventDetail, len(list))
...
// UnionEventDetailByID return a set that is the union of the input sets
// repeated value within list parameter will be ignored
func UnionEventDetailByID(arrList ...[]EventDetail) []EventDetail {
...
// repeated value within list parameter will be ignored
func UnionEventDetailByIDPtr(arrList ...[]*EventDetail) []*EventDetail {
	resultMap := make(map[int64]bool)
	var resultArr []*EventDetail
	for _, arr := range arrList {
		for _, v := range arr {
...
// IntersectionEventDetailByID return a set that is the intersection of the input sets
// repeated value within list parameter will be ignored
func IntersectionEventDetailByID(arrList ...[]EventDetail) []EventDetail {
	if arrList == nil {
		return []EventDetail{}
	}
...
// IntersectionEventDetailByIDPtr return a set that is the intersection of the input sets
// repeated value within list parameter will be ignored
func IntersectionEventDetailByIDPtr(arrList ...[]*EventDetail) []*EventDetail {
	if arrList == nil {
		return []*EventDetail{}
	}
...
// DifferenceEventDetailByID returns a set that is the first set without elements of the remaining sets
// repeated value within list parameter will be ignored
func DifferenceEventDetailByID(arrList ...[]EventDetail) []EventDetail {
	if arrList == nil {
		return []EventDetail{}
	}
...
// DifferenceEventDetailByIDPtr returns a set that is the first set without elements of the remaining sets
// repeated value within list parameter will be ignored
func DifferenceEventDetailByIDPtr(arrList ...[]*EventDetail) []*EventDetail {
	if arrList == nil {
		return []*EventDetail{}
	}
...
// SubsetEventDetailByID returns true or false by checking if set1 is a subset of set2
// repeated value within list parameter will be ignored
func SubsetEventDetailByID(list1, list2 []EventDetail) bool {
	if list1 == nil || len(list1) == 0 || list2 == nil || len(list2) == 0 {
		return false
	}
...
// SubsetEventDetailByIDPtr returns true or false by checking if set1 is a subset of set2
// repeated value within list parameter will be ignored
func SubsetEventDetailByIDPtr(list1, list2 []*EventDetail) bool {
	if list1 == nil || len(list1) == 0 || list2 == nil || len(list2) == 0 {
		return false
	}
...
// SupersetEventDetailByID returns true or false by checking if set1 is a superset of set2
// repeated value within list parameter will be ignored
func SupersetEventDetailByID(list1, list2 []EventDetail) bool {
	if list1 == nil || len(list1) == 0 || list2 == nil || len(list2) == 0 {
		return false
	}
...
// SupersetEventDetailByIDPtr returns true or false by checking if set1 is a superset of set2
// repeated value within list parameter will be ignored
func SupersetEventDetailByIDPtr(list1, list2 []*EventDetail) bool {
	if list1 == nil || len(list1) == 0 || list2 == nil || len(list2) == 0 {
		return false
	}
...
// SetEventDetailByID returns a set of the distinct elements of coll.
func SetEventDetailByID(list []EventDetail) []EventDetail {
...
// SetEventDetailByIDPtr returns a set of the distinct elements of coll.
func SetEventDetailByIDPtr(list []*EventDetail) []*EventDetail {
	if list == nil || len(list) == 0 {
		return []*EventDetail{}
	}
...
// UnionEventDetailByUpdatedAt return a set that is the union of the input sets
// repeated value within list parameter will be ignored
func UnionEventDetailByUpdatedAt(arrList ...[]EventDetail) []EventDetail {
	resultMap := make(map[time.Time]bool)
	var resultArr []EventDetail
	for _, arr := range arrList {
		for _, v := range arr {
...
// Union<FTYPE>Ptr return a set that is the union of the input sets
// repeated value within list parameter will be ignored
func UnionEventDetailByUpdatedAtPtr(arrList ...[]*EventDetail) []*EventDetail {
	resultMap := make(map[time.Time]bool)
	var resultArr []*EventDetail
	for _, arr := range arrList {
		for _, v := range arr {
...
// IntersectionEventDetailByUpdatedAt return a set that is the intersection of the input sets
// repeated value within list parameter will be ignored
func IntersectionEventDetailByUpdatedAt(arrList ...[]EventDetail) []EventDetail {
	if arrList == nil {
		return []EventDetail{}
	}
...
// IntersectionEventDetailByUpdatedAtPtr return a set that is the intersection of the input sets
// repeated value within list parameter will be ignored
func IntersectionEventDetailByUpdatedAtPtr(arrList ...[]*EventDetail) []*EventDetail {
	if arrList == nil {
		return []*EventDetail{}
	}
...
// DifferenceEventDetailByUpdatedAt returns a set that is the first set without elements of the remaining sets
// repeated value within list parameter will be ignored
func DifferenceEventDetailByUpdatedAt(arrList ...[]EventDetail) []EventDetail {
	if arrList == nil {
		return []EventDetail{}
	}
...
// DifferenceEventDetailByUpdatedAtPtr returns a set that is the first set without elements of the remaining sets
// repeated value within list parameter will be ignored
func DifferenceEventDetailByUpdatedAtPtr(arrList ...[]*EventDetail) []*EventDetail {
	if arrList == nil {
		return []*EventDetail{}
	}
...
// SubsetEventDetailByUpdatedAt returns true or false by checking if set1 is a subset of set2
// repeated value within list parameter will be ignored
func SubsetEventDetailByUpdatedAt(list1, list2 []EventDetail) bool {
	if list1 == nil || len(list1) == 0 || list2 == nil || len(list2) == 0 {
		return false
	}
...
// SubsetEventDetailByUpdatedAtPtr returns true or false by checking if set1 is a subset of set2
// repeated value within list parameter will be ignored
func SubsetEventDetailByUpdatedAtPtr(list1, list2 []*EventDetail) bool {
	if list1 == nil || len(list1) == 0 || list2 == nil || len(list2) == 0 {
		return false
	}
...
// SupersetEventDetailByUpdatedAt returns true or false by checking if set1 is a superset of set2
// repeated value within list parameter will be ignored
func SupersetEventDetailByUpdatedAt(list1, list2 []EventDetail) bool {
	if list1 == nil || len(list1) == 0 || list2 == nil || len(list2) == 0 {
		return false
	}
...
// SupersetEventDetailByUpdatedAtPtr returns true or false by checking if set1 is a superset of set2
// repeated value within list parameter will be ignored
func SupersetEventDetailByUpdatedAtPtr(list1, list2 []*EventDetail) bool {
	if list1 == nil || len(list1) == 0 || list2 == nil || len(list2) == 0 {
		return false
	}
...
// SetEventDetailByUpdatedAt returns a set of the distinct elements of coll.
func SetEventDetailByUpdatedAt(list []EventDetail) []EventDetail {
	if list == nil || len(list) == 0 {
		return []EventDetail{}
	}
...
// SetEventDetailByUpdatedAtPtr returns a set of the distinct elements of coll.
func SetEventDetailByUpdatedAtPtr(list []*EventDetail) []*EventDetail {
	if list == nil || len(list) == 0 {
		return []*EventDetail{}
	}
