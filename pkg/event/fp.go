...
func DistinctEventDetail(list []EventDetail) []EventDetail {
	var newList []EventDetail
	s := make(map[EventDetail]struct{}, len(list))
	for _, v := range list {
		if _, ok := s[v]; ok {
			continue
		}
...
// DistinctEventDetailPtr removes duplicates.
...
func UnionEventDetail(arrList ...[]EventDetail) []EventDetail {
	resultMap := make(map[EventDetail]bool)
	...
	resultArr := make([]EventDetail, len(resultMap))
	i := 0
